...
def main(config=None):
    ...
    for cv, (train_idx, val_idx) in enumerate(skf.split(train_df[["idx"]], train_df["nod"])):
        ...
        for epoch in range(config['epochs']):
            ...
            # train for one epoch
            train_log = utils.loop(config, train_loader, model, criterion, optimizer)
            print("Train: ", end="")
            for idx, (k, v) in enumerate(train_log.items()):
                ending = "\n" if idx == len(train_log)-1 else " - "
                print(k, round(v, 4), end=ending)

            val_log = utils.loop(config, val_loader, model, criterion, training=False)
        ...
        test_log = utils.loop(config, test_loader, model, criterion, training=False)
